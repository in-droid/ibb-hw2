from ultralytics import YOLO
import os, itertools, datetime
import pickle
# from tqdm import tqdm
# # Load a model
# model = YOLO("yolov8n.yaml")  # build a new model from scratch
# model = YOLO("yolov8n.pt")  # load a pretrained model (recommended for training)

# Move to the dir of this script        
dname = os.path.dirname(os.path.abspath(__file__))
os.chdir(dname)

# Load the model, feel free to try other models
model = YOLO("yolov8n.pt")

# learning_rates = [xx, xx, xx]
# dropouts = [xx, xx, xx, xx]
# weight_decays = [xx, xx,xx]

learning_rates = [0.0001, 0.001, 0.01]
dropouts = [0.01, 0.1, 0.2, 0.5]
weight_decays = [0.0001, 0.0005, 0.005, 0.05]
optimizers = ['SGD', 'Adam']

param_combinations = list(itertools.product(learning_rates, 
                                            dropouts, 
                                            weight_decays, 
                                            optimizers))

best_map = 0
best_params = None
all_metrics = {}
# Dear colleagues, for the full list and explanations, refer to: https://docs.ultralytics.com/usage/cfg/#train
for lr, dropout, wd, optimizer in param_combinations:
    model.train(
        data="ears.yaml", 
        epochs=20, 
        optimizer=optimizer, 
        pretrained=True, 
        patience=3,
        plots=True,
        batch=32,
        val=True,
        augment=False,
        dropout=dropout,
        lr0=lr,
        lrf=0.2,
        momentum=0.937,
        weight_decay=wd,
        warmup_epochs=3,
        warmup_momentum=0.5,
        warmup_bias_lr=0.1,
        box=7.5,
        cls=0.5
    )
    metrics = model.val() # It'll automatically evaluate the data you trained.
    all_metrics[(lr, dropout, wd, optimizer)] = metrics
    if metrics.results_dict['metrics/mAP50-95(B)'] > best_map:
        best_map = metrics.results_dict['metrics/mAP50-95(B)']
        best_params = (lr, dropout, wd, optimizer)
    print("Best mAP so far: ", best_map, " with params: ", best_params)

with open('all_metrics.pkl', 'wb') as f:
    pickle.dump(all_metrics, f)

print("Best mAP: ", best_map, " with params: ", best_params)


